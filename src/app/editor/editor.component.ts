...
export class EditorComponent implements OnInit {
  ...
  constructor(private http: HttpClient, private route: ActivatedRoute, private editorService: EditorService,
    ...
    if(this.id  != null && this.id !== "contact") {
      ...
      this.editorService.getPostForEdit(this.id).subscribe(res => {
        ...
        this.editform.setValue({ editor:res.content.content, title:res.content.title, description:res.content.description,
          ...
          videoUrl:res.content.videoUrl, subject:res.content.subject, subTopic:res.content.subTopic })
      });
    }
    else {
      this.mode = 'create';
  ...
  onSubmit() {
    ...
    let subject = this.editform.value.subject;
    let videoUrl = this.editform.value.videoUrl;
    let subTopic = this.editform.value.subTopic;
    let data = { content:content, title:title, description:description, keyword:keyword, type:type, nextPage:nextPage, previousPage:previousPage, videoUrl:videoUrl, subject:subject, subTopic:subTopic }
    ...
    if(this.mode==='edit') {
      let postData:Content = {_id:this.id, content:this.editform.value.editor, title:this.editform.value.title, description:this.editform.value.description, keyword:this.editform.value.keyword, type:this.editform.value.type,
        nextPage:this.editform.value.nextPage, previousPage:this.editform.value.previousPage, videoUrl:this.editform.value.videoUrl, subject:this.editform.value.subject,  subTopic:this.editform.value.subTopic }
